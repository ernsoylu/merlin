...
### Package 7K — bounded GPTimer release option — 2026-09-20

Complete for the current software-only scope. `Mcal_Gpt` now retains the
monotonic timebase and adds a GPTimer alarm contract with validated resolution
and period, periodic auto-reload, ISR-context callback delivery, start/stop,
and explicit release that disables and deletes the timer. Host coverage checks
configuration rejection and unavailable-target behavior; the ESP32 component
